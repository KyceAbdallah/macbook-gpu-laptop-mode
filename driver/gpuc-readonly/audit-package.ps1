...
if ($infExists) {
    ...
    Write-Output ("  Hardware match ACPI\APP000B: " + ($infText -match [regex]::Escape("ACPI\APP000B")))
    Write-Output ("  Broad ACPI wildcard match: " + ($infText -match "ACPI\\\*|ACPI\\.*\*"))
    Write-Output ("  ServiceType=1: " + ($infText -match "ServiceType\s*=\s*1"))
    Write-Output ("  StartType=3: " + ($infText -match "StartType\s*=\s*3"))
    Write-Output ("  KmdfLibraryVersion=1.15: " + ($infText -match "KmdfLibraryVersion\s*=\s*1\.15"))
    Write-Output ("  CatalogFile declared: " + ($infText -match "CatalogFile\s*=\s*gpuc-readonly\.cat"))
}
